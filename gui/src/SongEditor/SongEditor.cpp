...
void SongEditor::mousePressEvent( QMouseEvent *ev )
{
	...
	PatternList *pPatternList = pSong->get_patterns();
...
void SongEditor::mouseMoveEvent(QMouseEvent *ev)
{
	int nRow = ev->y() / m_nGridHeight;
	int nColumn = ( (int)ev->x() - 10 ) / (int)m_nGridWidth;
	PatternList *pPatternList = Hydrogen::get_instance()->getSong()->get_patterns();
...
void SongEditor::createBackground()
{
	...
	uint nPatterns = pSong->get_patterns()->size();
...
void SongEditor::drawSequence()
{
	...
	for (uint i = 0; i < pColumns->size(); i++) {
		...
		for (uint nPat = 0; nPat < pColumn->size(); ++nPat) {
			...
			for (Pattern::virtual_patterns_cst_it_t virtualIter = pat->get_flattened_virtual_patterns()->begin(); virtualIter != pat->get_flattened_virtual_patterns()->end(); ++virtualIter) {
...
void SongEditorPatternList::createBackground()
{
	...
	Hydrogen *pEngine = Hydrogen::get_instance();
	Song *pSong = pEngine->getSong();
	int nPatterns = pSong->get_patterns()->size();
	int nSelectedPattern = pEngine->getSelectedPatternNumber();
	...
	for ( int i = 0; i < nPatterns; i++ ) {
		H2Core::Pattern *pPattern = pSong->get_patterns()->get(i);
		//uint y = m_nGridHeight * i;
...
void SongEditorPatternList::patternPopup_virtualPattern()
{
    ...
    if ( dialog->exec() == QDialog::Accepted ) {
	    selectedPattern->clear_virtual_patterns();
	    for (unsigned int index = 0; index < listsize-1; ++index) {
	        ...
	        if (dialog->patternList->isItemSelected(listItem) == true) {
		        if (patternNameMap.find(listItem->text()) != patternNameMap.end()) {
                    selectedPattern->add_virtual_pattern(patternNameMap[listItem->text()]);
		        }
	        }
	    }
        pSEPanel->updateAll();
    ...
    pPatternList->compute_flattened_virtual_patterns();
...
void SongEditorPatternList::patternPopup_load()
{
	...
	Song *song = engine->getSong();
	Pattern *pat = song->get_patterns()->get( tmpselectedpatternpos );
...
{	
	...
	Song *song = engine->getSong();
	Pattern *pat = song->get_patterns()->get( nSelectedPattern );
...
void SongEditorPatternList::patternPopup_delete()
{
	...
	Hydrogen *pEngine = Hydrogen::get_instance();
	Song *song = pEngine->getSong();
	PatternList *pSongPatternList = song->get_patterns();
...
void SongEditorPatternList::deletePatternFromList( QString patternFilename, QString sequenceFileName, int patternPosition )
{
	...
	Pattern *pat = song->get_patterns()->get( patternPosition );
    ...
    pSongPatternList->del_virtual_pattern( pattern );
...
void SongEditorPatternList::patternPopup_copy()
{
	...
	if ( dialog->exec() == QDialog::Accepted ) {
		...
		SE_copyPatternAction *action = new SE_copyPatternAction( patternFilename , pPatternList->size() );
		HydrogenApp::get_instance()->m_undoStack->push( action );
	}
...
void SongEditorPatternList::dropEvent(QDropEvent *event)
{
	...
	if (sText.startsWith("move pattern:")) {
		...
		//create a unique sequencefilename
		Song *song = Hydrogen::get_instance()->getSong();
		Pattern *pat = song->get_patterns()->get( nTargetPattern );
...
void SongEditorPatternList::movePatternLine( int nSourcePattern , int nTargetPattern )
{
		...
		pSong->get_patterns()->move(nSourcePattern, nTargetPattern );
