...
void Hydrogen::stopMidiClockTimer()
{
    delete m_pMidiClockTimer;
    m_pMidiClockTimer = NULL;
}
...
void Hydrogen::sequencer_stop()
{
	...
	if( Preferences::get_instance()->get_sendMidiSpp() == true){
		...
		position = m_pAudioDriver->m_transport.m_nFrames / m_pAudioDriver->m_transport.m_nTickSize / 12;
                if( m_bExportsong == false )
                    getMidiOutput()->handleSongPosition( position );
	}
	//handle Midi Stop
        if( Preferences::get_instance()->get_sendMidiStartStop() == true && m_bExportsong == false ){
...
#ifdef H2CORE_HAVE_ALSA
                if( Preferences::get_instance()->get_sendMidiClock() == true && m_bExportsong == false ) {
			fBPM = round(fBPM);
